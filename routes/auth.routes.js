...
router.post("/signup", async(req, res, next) => {
    ...
    if (!email || !password || !username) {
        res.status(400).json({errorMessage: "Email, Password and username are required"})
        return // this is to stop the execution of the route
    }

    // the strength of the password
    const passwordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/gm
    if (passwordRegex.test(password) === false) {
        res.status(400).json({errorMessage: "Password is not enough. Needs to have at least one digit, one uppercase, one lowercase and 8 characters of length."})
        return // this is to stop the execution of the route
    }
    ...
    try {
        ...
        //* BONUS
        // validate date types for all properties
        // format of the email to be correct

        // we will create the document and send am ok message
...
router.post("/login", async(req, res, next) => {
    ...
    if (!email || !password) {
        ...
        return // this is to stop the executive of the route
    }
